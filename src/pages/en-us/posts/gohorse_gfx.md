...
date: 2023-08-31
modified: 2023-09-06
tags: project,management,agile,principles,gohorse,error,failure,waterfall,graphic,team
---
...
- [Breeding Horses](#breeding-horses)
    - [A tale as Old as the Company](#a-tale-as-old-as-the-company)
...
> If you like sarcasm and want to taint your mind with the definition, [check the GoHorse axioms at your own peril](https://medium.com/@dekaah/22-axioms-of-the-extreme-go-horse-methodology-xgh-9fa739ab55b4){target="_blank"}.
...
It is not simply a _"throw money to solve"_ kind of problem. It requires `dedication`, `attention` and `work ethics`.

Do you realize now why `GoHorse` is so prevalent in today's world?

### <i class="fa-solid fa-maximize"></i> Too Big to Fail

As we have seen in `agile`, it takes a lot of effort to maintain the `team` united, that's one of the reasons `agile teams` tend to be small.
`Waterfall` doesn't really seem to care about `team size` because the most important thing is the `process` so it ends up being a `indirect concern`.
...
Since some `projects` are more of a desire, they end up poorly planned. At least in my experience as a `software engineer` this is so very common it is kind of an `industry joke` at this point.
...
### <i class="fa-solid fa-business-time"></i> A tale as Old as the Company

Not only `budget constraints` cause `GoHorse projects` to exist. Sometimes `inertia` may play against the `projects` as well.

There are many businesses over 30 years old still going strong. Some are even older than that. Sure, they _may_ have gotten a `rebranding` here, maybe broken up in `smaller entities` there but in essence, they still have that old spirit in them.
...
As you may have guessed by the hints I left, companies such as those are `very resistive to change` and would rather do what they have always done, regardless of the result.
...
`Agile teams` [have to become self sufficient](project_agile.html#self-improvement-is-real){target="_blank"} in order to [have high performance](#the-team-makes-the-dream-come-true) as well as [making sure everyone is connected](#avoid-e-mails-that-could-be-a-conversation).

Those `principles` are easy to lose track of, especially if your `team` get too big. What does it really mean, `to have a team too big`? You know you have reached `critical mass` when `managers do not have enough time in their calendars to have a 15-minute conversation with their personnel at least once a month`.
...
I have named this article `GoHorse GTX` because of the [graphical style](#graphics-are-more-important-than-the-gameplay) I have seen going around, I myself got caught in some `projects` using this.

But hey, **there really is a light at the end of the tuneel**: make sure you also [know the agile framework](project_agile.html){target="_blank"} and even the [waterfall method](project_waterfall.html) too, why not?

Make sure you understand them and fight for them: only you can stop `GoHorse` from being a regular occurence.
